...
- [x] T001 Criar migration aditiva `api/migrations/0011-special-needs-description.sql` com
...
- [x] T002 Aplicar a migration localmente com
...
- [x] T003 [P] Atualizar `CandidateApplicationRow` e `NewCandidateApplication` em
      `shared/src/schemas/database.schema.ts` com `special_needs_description: string | null`
- [x] T004 [P] Em `shared/src/schemas/candidate.schema.ts`: extrair `AvailabilityStepFields`
...
- [x] T005 [P] Em `shared/src/schemas/dashboard.schema.ts`: adicionar
...
- [x] T006 Rodar `npx tsc --noEmit --project shared/tsconfig.json` e confirmar que compila
...
- [x] T007 [P] [US1] Em `api/test/candidates.service.test.ts`: adicionar casos —
...
- [x] T008 [P] [US1] Em `api/test/candidates.routes.test.ts`: adicionar caso — `POST
      /candidate/register` com `specialNeeds: true` e `specialNeedsDescription` ausente ou
      só espaços retorna `400` com issue em `specialNeedsDescription`; caso com descrição de
      501 caracteres também retorna `400`. (Nota de execução: a rota real usa `400` para
      erro de validação de shape do Zod, não `422` como o contrato inicial supunha — mesmo
      código já usado para os demais casos de validação desta rota, ex. `referralSourceOther`
      ausente. Ajustado para seguir o padrão real em vez de introduzir um código novo.)

### Implementation for User Story 1

- [x] T009 [US1] Em `api/src/repositories/candidates.repository.ts`: adicionar
...
- [x] T010 [US1] Em `api/src/services/candidates.service.ts`: montar
...
- [x] T011 [US1] Em `api/src/services/sheet-sync.service.ts`: incluir a coluna
...
- [x] T012 [P] [US1] Em `front/app/inscricao/_components/availability-step-form.tsx`:
...
- [x] T013 [US1] Em `front/app/inscricao/_lib/wizard-guards.ts`: atualizar `isStepComplete`
...
- [x] T014 [US1] Rodar `npm run test --workspace=api` e confirmar que os testes de T007/T008
      passam
...
- [x] T015 [P] [US2] Em `api/test/dashboard.service.test.ts`: adicionar caso — `detail()`
...
- [x] T016 [P] [US2] Em `api/test/dashboard.routes.test.ts`: adicionar caso — `GET
...
- [x] T017 [US2] Em `api/src/repositories/dashboard.repository.ts`: adicionar
...
- [x] T018 [US2] Em `api/src/services/dashboard.service.ts`: mapear
      `row.special_needs_description` para `application.specialNeedsDescription` no retorno
      de `detail()`
- [x] T019 [US2] Em `front/app/painel/_components/candidate-detail-sheet.tsx`: exibir a
...
- [x] T020 [US2] Rodar `npm run test --workspace=api` e confirmar que os testes de
      T015/T016 passam
...
- [x] T021 [P] [US3] Em `api/test/dashboard.service.test.ts`: adicionar caso — `metrics()`
...
- [x] T022 [P] [US3] Em `api/test/dashboard.routes.test.ts`: adicionar caso — `GET
...
- [x] T023 [P] [US3] Em `api/test/checkin.routes.test.ts`: adicionar caso — a resposta de
...
- [x] T024 [US3] Rodar `npm run test --workspace=api` e confirmar que os testes de
...
- [x] T025 [P] Rodar `npx tsc --noEmit --project api/tsconfig.json` e
...
- [x] T026 Rodar `npm run build --workspace=front` e confirmar build limpo
- [x] T027 Rodar a suíte completa (`npm run test --workspace=api`) uma última vez, todos os
      testes verdes
- [x] T028 Executar a validação end-to-end via `curl` descrita em `quickstart.md` (passos
      4 e 5) contra `wrangler dev` local — sem abrir o Browser pane sem autorização explícita
- [x] T029 Atualizar `api/scripts/seed-local.sql` com valores de exemplo para
...
- [x] T030 Revisar `specs/014-necessidades-especiais-descricao/checklists/requirements.md`
