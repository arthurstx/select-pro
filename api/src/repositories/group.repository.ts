 ...
 */
function normalizeMemberStatusRow<
  T extends { memberStatus: string; user_id: string },
>(row: T): T & { memberStatus: MemberStatus } {
  const memberStatus = normalizeStoredMemberStatus(row.memberStatus);
  if (!memberStatus) {
    logger.error("group_repository.unknown_member_status", {
      userId: row.user_id,
      raw: row.memberStatus,
    });
    throw new Error(
      `memberStatus desconhecido para o usuário ${row.user_id}: ${row.memberStatus}`,
    );
  }
...
export class GroupRepository {
   ...
   */
  async listPresentCandidates(
    processId: string,
  ): Promise<PresentCandidateRow[]> {
  ...
  async replaceOrganization(
    ...
    const statements: D1PreparedStatement[] = [
      this.db
        .prepare("DELETE FROM groups WHERE process_id = ? AND modality = ?")
        .bind(processId, modality),
    ];
    ...
    for (const group of groups) {
      statements.push(
        this.db
          .prepare(
            "INSERT INTO groups (id, process_id, room_id, modality, name) VALUES (?, ?, ?, ?, ?)",
          )
      ...
      for (const candidateId of group.candidateIds) {
        statements.push(
          this.db
            .prepare(
              "INSERT INTO group_candidates (group_id, candidate_id) VALUES (?, ?)",
            )
  ...
  async listCandidateAllocations(
    processId: string,
  ): Promise<GroupCandidateAllocationRow[]> {
    const { results } = await this.db
      .prepare(
  ...
  async listEvaluatorAllocations(
    processId: string,
  ): Promise<GroupEvaluatorAllocationRow[]> {
    const { results } = await this.db
      .prepare(
  ...
  async listCandidateAllocationsForGroup(
    groupId: string,
  ): Promise<GroupCandidateAllocationRow[]> {
    const { results } = await this.db
      .prepare(
  ...
  async listEvaluatorAllocationsForGroup(
    groupId: string,
  ): Promise<GroupEvaluatorAllocationRow[]> {
    const { results } = await this.db
      .prepare(
  ...
  async moveCandidate(candidateId: string, toGroupId: string): Promise<void> {
    await this.db
      .prepare(
        "UPDATE group_candidates SET group_id = ? WHERE candidate_id = ?",
      )
  ...
  async moveEvaluator(userId: string, toGroupId: string): Promise<void> {
    await this.db
      .prepare("UPDATE group_evaluators SET group_id = ? WHERE user_id = ?")
  ...
  async removeEvaluator(userId: string): Promise<boolean> {
    const result = await this.db
      .prepare("DELETE FROM group_evaluators WHERE user_id = ?")
      .bind(userId)
      .run();
